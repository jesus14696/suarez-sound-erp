...
# --- GESTIÓN DE SESIÓN Y FILTRO DE ACCESO ---
if "user" not in st.session_state:
    st.session_state.user = None

# Capturar el token cuando Google redirige de vuelta a la app
params = st.query_params
if "access_token" in params and not st.session_state.user:
    ...
    try:
        ...
        st.error(f"Error al validar la sesión: {e}")

# Si el usuario NO ha iniciado sesión, mostramos la pantalla de bloqueo con Google
if st.session_state.user is None:
    st.title("🔒 Acceso Restringido - Suárez Sound")
    ...
    try:
        ...
        st.error(f"Error generando el enlace de autenticación: {e}")
        
    # Detenemos la ejecución para que no se cargue el resto del ERP
    st.stop()

# Si ya está autenticado, agregamos opción en la barra lateral para cerrar sesión
user = st.session_state.user
st.sidebar.success(f"Conectado: {user.email}")
